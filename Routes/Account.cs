...
public dynamic ReturnGetBio(long accountId)
{
    ...
    return new BioDTO
    {
        ...
        public async Task<dynamic> ReturnCreateAccount(HttpContext ctx)
        {
            string? platformIdValue = ctx.Request.Form["platformId"];
            if (string.IsNullOrWhiteSpace(platformIdValue))
                platformIdValue = ctx.Request.Form["platform_id"];

            if (!ulong.TryParse(platformIdValue, out ulong platformId))
                return Results.StatusCode(403);

            string? platformValue = ctx.Request.Form["platform"];
            if (!int.TryParse(platformValue, out int platformInt) || !Enum.IsDefined(typeof(PlatformType), platformInt))
                return Results.StatusCode(403);

            PlatformType platform = (PlatformType)platformInt;

            string alias;
            if (platform == PlatformType.Oculus)
            {
                OculusServer.User? oculusUser = await OculusServer.GetUserAsync(platformId);
                if (oculusUser?.Alias == null)
                    return Results.StatusCode(403);

                alias = oculusUser.Alias;
            }
            else
            {
                alias = $"Player{platformId % 1_000_000_000}";
            }
            ...
            MongoDB.RoomDetailsMongoDB roomDetails = new()
            {
                Room = new()
                {
                    RoomId = MongoDB.roomsCollection.CountDocuments(_ => true) + 1,
                    Name = $"@{alias}'s Dorm",
