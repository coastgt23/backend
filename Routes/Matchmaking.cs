...
namespace Stella.Routes
{
    public class Matchmaking
    {
        ...
        [ServerAPI.POST("/match/player/logout")]
        [ServerAPI.UseAuthorization]
        public async Task<RecNetResult> ReturnLogout(MongoDB.User user)
        {
            MongoDB.usersCollection.UpdateOne(
                u => u.AccountId == user.AccountId,
                Builders<MongoDB.User>.Update.Set(u => u.OtherData.RoomInstance, null)
            );
...
public async Task<dynamic> ReturnGotoRoom(string roomName, MongoDB.User user)
{
    ...
    if (roomName.Equals("dormroom", StringComparison.CurrentCultureIgnoreCase))
    {
        ...
        if (room == null)
        {
            room = Rooms.CreateDefaultRoom((int)user.OtherData.DormRoomId, "DormRoom", (int)user.AccountId);
    ...
    else
    {
        ...
        if (room == null)
        {
            ...
            room = Rooms.CreateDefaultRoom(newId, roomName, (int)user.AccountId);
            MongoDB.roomsCollection.InsertOne(room);
        }
    }
